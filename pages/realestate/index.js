...
const testvts = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchkey) {
      router.push({ pathname: `/realestate/search`, query: { 'searchKey': searchkey } })
    } else {
      // router.push({
                      ...
                      <div className="recommend_theme h-100">
                        <div className="d-flex justify-content-between my-3"><h3 className="fw-bolder">{t('Recommended Properties')}</h3>
                          {properties?.data?.recommended?.length > 6 && <h5 className="foot-cat cursor-pointer text-capitalize text-end fw-bolder"
                            onClick={() => router.push({ pathname: `/realestate/${'recommended'}`,
                            //  query: { 'prop': 'Recommended' }
                              })}>{t('view More')}</h5>} </div>
                        <div className="">
                          <Row>
                            {
                                ...
                                <Col md="6" xl="6" lg="12 h-100" className='mb-3'  key={index}>
                                  <div className="bg-form-content h-100">
                                    <Row>
                                      <Col xs="4 px-2" xl='5' >
                                        <div className="w-100 h-100 position-relative">
                                          <img className=" img-form-icon cursor-pointer img-fit w-100" src={value?.item_image_medium}
                                            ...
                                            onClick={() => router.push({ pathname: `/realestate/property/${value?.item_slug}`,
                                            //  query: { 'property_id': value?.id }
                                             })}
                                          />
                                      ...
                                      <Col>
                                        <Link href={{ pathname: `/realestate/property/${value?.item_slug}`,
                                        //  query: { 'property_id': value?.id }
                                          }}>
                    ...
                    <Col lg="6 mb-3">
                      ...
                      <div className="recommend_theme h-100">
                        <div className="d-flex justify-content-between my-3"><h3 className="fw-bolder">{t('Popular Properties')}</h3>
                          {properties?.data?.Popular?.length > 6 && <h5 className="foot-cat cursor-pointer text-capitalize fw-bolder text-end" onClick={() => router.push({ pathname: `/realestate/${'popular'}`,
                        //    query: { 'prop': 'Popular' }
                            })}>{t('view More')}</h5>} </div>
                        <div className="">
                          <Row>
                ...
                <div className="section-trending mb-4 mt-3">
                  ...
                  <div className="recommend_theme h-100">
                    <div className="d-flex justify-content-between my-3"><h3 className="fw-bolder">{t('Trending Properties')}</h3>
                      {properties?.data?.trending?.length > 6 && <h5 className="foot-cat cursor-pointer fw-bolder text-end" onClick={() => router.push({
                         pathname: `/realestate/${'trending'}`,
                        //   query: { 'prop': 'Trending' } 
                          })}>{t('View All')}</h5>} </div>
                    <div className="">
                      <Row>
